...
func mergeResources(oldSpec *unstructured.Unstructured, componentName string) (interface{}, bool) {
	...
	if apiFound && !backendFound {
		...
		return migrationutils.ResourceRequirementsToUnstructured(aRR), true
	}
	...
	if !apiFound && backendFound {
		...
		return migrationutils.ResourceRequirementsToUnstructured(bRR), true
	}
	...
	if apiFound && backendFound {
		...
		return migrationutils.ResourceRequirementsToUnstructured(mergedRes), true
	}
...
func convertAstarteGenericComponentSpec(oldSpec *unstructured.Unstructured, componentName string) (newComponent *unstructured.Unstructured) {
	...
	if (apiFound == false || oldAPI == nil) && (backendFound == false || oldBackend == nil) {
	...
	if apiFound && (backendFound == false || oldBackend == nil) {
	...
	if backendFound && (apiFound == false || oldAPI == nil) {
	...
	if mergedRes, found := mergeResources(oldSpec, componentName); found {
